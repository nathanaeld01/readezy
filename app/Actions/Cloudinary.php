...
class Cloudinary {
	...
	/**
	 * Generates a Cloudinary URL for the specified public ID.
	 *
	 * @param string $publicId The public ID of the file to be displayed.
	 * @param string|null $transforms The transformation preset to apply (optional).
	 *
	 * @return string The URL of the file with the specified transformations.
	 */
	...
	public static function exists(string $publicId, ?bool $omitExtension = false) {
		if (!str_starts_with($publicId, 'readezy/')) {
			$publicId = "readezy/" . $publicId;
		}
		...
		return $resource['public_id'] . ($omitExtension ? '' : ".{$resource['format']}");
	}

	/**
	 * Retrieves the public ID of a file from its path.
	 *
	 * @param string $path The path to the file.
	 *
	 * @return string The public ID of the file.
	 */
